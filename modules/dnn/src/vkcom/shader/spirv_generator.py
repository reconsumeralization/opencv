...
with open('spv_shader.hpp', 'w') as headfile:
    ...
    for i in range(len(list)):
        ...
        bin_file = f'{prefix}.tmp'
        cmd = f' glslangValidator -V {path} -S comp -o {bin_file}'
        print('compiling')
        if os.system(cmd) != 0:
            continue;
        size = os.path.getsize(bin_file)

        spv_txt_file = f'{prefix}.spv'
        cmd = f'glslangValidator -V {path} -S comp -o {spv_txt_file} -x{null_out}'
        os.system(cmd)

        infile_name = spv_txt_file
        outfile_name = f'{prefix}_spv.cpp'
        array_name = f'{prefix}_spv'
        ...
        os.system(f'{cmd_remove} {bin_file}')
        os.system(f'{cmd_remove} {spv_txt_file}')
